...
- Alert words (settings dialog): if a client's title *changes* and contains one of the words, BotWall beeps + toasts.
...
- Settings persistence via `QSettings("BotWall", "BotWall")`: window geometry, card size (zoom), sort mode, CPU mode, pinned titles, nicknames (JSON, keyed by title), scan keywords, alert words.
